...
When `pacman(8)` is broken, either by partial upgrades or mirror errors, and you don't have a spare USB flash drive for the boot ISO, you can manually install `.zst` packages to _temporarily_ fix broken dependencies. And no, this is not `pacman -U`, and did not involve the Arch Linux Archive.
